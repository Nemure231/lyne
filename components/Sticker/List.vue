...
<template>
    ...
    <template v-if="loadingSticker">
        <div class="flex flex-wrap lg:justify-start md:justify-start justify-center items-center flex-row gap-6 lg:mt-0 md:mt-0
            mt-4">
            <div v-for="defaultLoop in 10" class="animate-pulse hover:bg-green-400 bg-gray-300 rounded-md flex-none lg:w-40
    ...
    <template v-else>
        <template v-if="item.data?.stickers?.length > 1 && item.status == 200">
            <OnClickOutside @trigger="focusScale = 0">
                <div
                    ...
                    <div v-for="pl in item.data?.stickers" :key="pl.id" :style="{ backgroundImage: `url(${pl.staticUrl})` }"
