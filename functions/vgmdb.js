...
exports.handler = async (event, context, callback) => {
    ...
    const url = `https://vgmdb.net`;

    const { data } = await axios.get(url)

    const $ = cheerio.load(data);

    const check = $(".releaseblock");

    const tex = []

    check.each(function (i, elm) {
        const data_sticker = $(this).find('[class="dateblock"]').text();
        tex.push({
            date: data_sticker,
            list: elm.children.text
        });

        // elm.find('[class="album_infobit_small"]').each(function (i, elm) {
